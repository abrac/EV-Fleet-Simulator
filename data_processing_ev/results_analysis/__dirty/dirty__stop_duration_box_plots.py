...
def main(scenario_dir: Path, plot_blotches: bool = False, figsize = (4,3)):
    ...
    save_dir = scenario_dir.joinpath('Temporal_Clusters', 'Clustered_Data',
                                     '_dirty__stop_duration_box_plots')
    save_dir.mkdir(parents=True, exist_ok=True)
    # Output box-plots.
    # As png:
    fig_dir = save_dir.joinpath("dirty__stop_duration_box_plots.png")
    plt.savefig(fig_dir)
    # As svg:
    fig_dir = save_dir.joinpath("dirty__stop_duration_box_plots.pdf")
    plt.savefig(fig_dir)
    # As pickle:
    fig_dir = save_dir.joinpath(
        "dirty__stop_duration_box_plots.fig.pickle")
    fig = plt.gcf()
    pickle.dump(fig, open(fig_dir, 'wb'))

    ev_stop_dfs_combined = pd.concat(summed_ev_stop_dfs)
    ev_stop_dfs_combined.to_csv(
        save_dir.joinpath('dirty__stop_duration_box_plots.csv')
    )
